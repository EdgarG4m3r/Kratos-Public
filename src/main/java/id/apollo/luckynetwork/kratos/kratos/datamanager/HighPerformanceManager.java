...
/**
 * HighPerformance manager for High performance. Transaction per second is rated for 3300 TPS up to 7500 TPS / thread.
 */
public class HighPerformanceManager {
    ...
    public HighPerformanceManager(Kratos kratos) {
        this.kratos = kratos;
    }
    ...
    private void call(String args) {
        kratos.getDataSaverService().execute(() -> {
            String sql = "CALL " + args;
            try(Connection connection = kratos.getMySQL().getConnection(); Statement statement = connection.createStatement()) {
                statement.execute(sql);
            } catch (Exception e) {
                e.printStackTrace();
                kratos.getLogger().log(Level.SEVERE, "There was an error executing the query: " + args);
                kratos.getLogger().log(Level.SEVERE, "The transaction has been cancelled");
    ...
    {
        sendRedisUpdate(uuid);
        kratos.getGlobalManager().reloadPlayer(uuid);
    }
    ...
    {
        if (!kratos.getRedisHandler().isRedisConnected())
        {
            kratos.reconnectRedis();
        }
        if (kratos.getRedisHandler().isRedisConnected())
        {
            JsonBuilder payload = new JsonBuilder()
                    .addProperty("playerUUID", uuid.toString());
            new PlayerUpdate(kratos, payload).broadcast();
        }
